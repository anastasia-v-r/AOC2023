    ...
    internal class Day1 : BaseDay
    {
        ...
        public override void Run()
        {
            ...
            foreach (string code in codes)
            {
                ...
                {
                    if (System.Char.IsDigit(index))
                    {
                        num.Add(index);
                    }
                }
                string value = num[0].ToString() + num[num.Count - 1].ToString();
                total += Int32.Parse(value);
            }

            Console.WriteLine("Final Value is : " + total.ToString());
        }
    }
}
...
do you think snow comes from") when you realize that the Elves are already loading you into a trebuchet ("please hold still, we need to strap you in").
...
the Elves now need to recover. On each line, the calibration value can be found by combining the first digit and the last digit (in that order) to form a single two-digit number.
